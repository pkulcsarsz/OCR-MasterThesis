...
def VGG(input_shape, num_classes, steps_per_epoch, epochs, use_cache=False, dataset='dataset1'):
    ...
    # define new model
    model = Model(inputs=model.inputs, outputs=classifierOutput)
    # summarize
    model.summary()
...
def customVGG(input_shape, num_classes, steps_per_epoch, epochs, use_cache=False, dataset='dataset1'):
    ...
    def featuresAccuracy(y_true, y_pred):
        print("Pred", y_pred[1,:n])
        print("True", y_true.numpy()[1,k:])
        print(y_pred[1,:n] - tf.cast(y_true, tf.float32)[1,k:])
    ...
    weights = {'classifierOutput':1.0, 'featuresOutput':100.0}
